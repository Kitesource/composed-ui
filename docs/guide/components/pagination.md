...
import { ref } from 'vue'
import type { PaginationProps } from 'composed-ui'
const page = ref(1)
...
jumperProps | 快速跳转组件 `props`，参考 [Input Props](https://Kitesource.github.io/composed-ui/guide/components/input.html#input) | object | {}
showSizeChanger | 是否展示 `pageSize` 切换器，当 `total` 大于 `50` 时默认为 `true` | boolean | undefined
changerProps | `pageSize` 切换器组件 `props`，参考 [Select Props](https://Kitesource.github.io/composed-ui/guide/components/select.html#select) | object | {}
