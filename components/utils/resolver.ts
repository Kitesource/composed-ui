...
function getSideEffects(componentName: string, options?: ComposeUIResolverOptions) {
  ...
  const sideEffects: string[] = [`composed-ui/${type}/style/global.css`] // 组件库全局默认样式
  sideEffectsComponents.forEach((component: string) => {
    sideEffects.push(`composed-ui/${type}/${componentsMap[component as keyof typeof componentsMap]}/${component}.css`)
  })
  if (componentName === 'DatePicker') {
    // 特殊处理 DatePicker 组件样式依赖文件
    sideEffects.push(
      `composed-ui/${type}/node_modules/.pnpm/@vuepic_vue-datepicker@11.0.2_vue@3.5.21_typescript@5.9.2_/node_modules/@vuepic/vue-datepicker/dist/main.css`
    )
  }
  if (componentName === 'Swiper') {
    // 特殊处理 Swiper 组件样式依赖文件
    sideEffects.push(`composed-ui/${type}/node_modules/.pnpm/swiper@11.2.10/node_modules/swiper/swiper.css`)
    ...
    swiperModulesStyle.forEach((moduleName) => {
      sideEffects.push(
        `composed-ui/${type}/node_modules/.pnpm/swiper@11.2.10/node_modules/swiper/modules/${moduleName}.css`
      )
    })
  }
  return sideEffects
}
export interface ComposeUIResolverOptions {
  cjs?: boolean // whether use commonjs build, default false
}
export function ComposeUIResolver(options?: ComposeUIResolverOptions) {
  return {
